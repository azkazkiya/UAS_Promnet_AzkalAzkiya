...
func GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	var users []User
	result, err := db.Query("SELECT id, date, description, amount, status, receiver, jk, no_telp, address from transaksi_keuangan_azkalazkiyaalfiandri")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO transaksi_keuangan_azkalazkiyaalfiandri (date, description, amount, status, receiver, jk, no_telp, address) VALUES (?,?,?,?,?,?,?,?)")
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	result, err := db.Query("SELECT id, date, description, amount, status, receiver, jk, no_telp, address from transaksi_keuangan_azkalazkiyaalfiandri WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE transaksi_keuangan_azkalazkiyaalfiandri SET date = ?, description = ?, amount = ?, status = ?, receiver = ?, jk = ?, no_telp = ?, address = ? WHERE id = ?")
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("DELETE FROM transaksi_keuangan_azkalazkiyaalfiandri WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql", "root:@tcp(127.0.0.1:3306)/db_2201752_azkalazkiyaalfiandri_uas_pilkomb")
	if err != nil {
		log.Fatal(err)
	}
